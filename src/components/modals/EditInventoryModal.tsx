...
const EditInventoryModal: React.FC<EditInventoryModalProps> = ({ isOpen, onClose, onUpdate, item }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const success = storageUtils.updateInventoryItem(item.id, {
      ...formData,
      status
    });

    if (success) {
      const updatedItem = { ...item, ...formData, status };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-lg font-semibold text-gray-800">Edit Inventory Item</h2>
          </div>
          <button
            onClick={onClose}
